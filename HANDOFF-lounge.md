...
| `me-too` | 저도요 | 🖼 손 들고 활짝 |
| `agree` | 동의 | 🖼 눈 뜨고 엄지척 |
| `laugh` | 빵터짐 | 🖼 눈 감고 웃으며 엄지척 |
| `goal` | 골! | 🖼 만세 환호 |
| `fire` | 불타오르네 | 🖼 눈썹 세우고 주먹 |
| `cry` | 슬퍼요 | 🖼 눈물 |
| `tired` | 힘들어요 | 🖼 땀 + 혀 |
| `sorry` | 죄송 | 🙏 |
| `thinking` | 음… | 🤔 |
...
그림 7장 합쳐 **40KB**다. 표정이 겹치면 작게 줄였을 때 구분이 안 되므로,
손 흔드는 그림이 둘이었을 때 더 또렷한 쪽만 남기고 하나는 뺐다.
...
**잘라내지 말고 정사각형에 통째로 맞춰 넣는다.** 머리만 크게 오려내면 손·공·땀방울처럼
표정을 거드는 것들이 잘려 나간다. 투명 여백만 걷어내고(알파 bbox) `fit: contain` 으로
정사각형에 넣으면 아무것도 안 잘린다.

**192px 팔레트 PNG.** 화면 최대 64px 이라 3배 화면에도 192px 면 충분하고, 색이 평평한
그림이라 팔레트(96색)로 넣으면 90KB → 6KB 로 줄면서 눈에 띄는 차이가 없다.
1254px 원본을 그대로 넣으면 38px 아이콘 하나에 1MB 를 받게 된다.

**표정을 확실히 다르게** 뽑아야 한다. 웃는 얼굴만 여러 장이면 "슬퍼요"를 만들 수가
없고, 작게 줄였을 때 서로 구분도 안 된다.

원본은 `emoticon/`(repo 루트)에 두고, 위 규칙대로 변환해 `public/emoticons/` 에 넣는다.
