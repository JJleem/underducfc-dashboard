...
export default function PlayerStatsReport({ report, attendance, season }: { report: StatsReport; attendance: { rate: number | null; count: number; total: number; streak: number; accent: string }; /** 보고 있는 시즌 라벨("25-26"). 지난 시즌을 열어도 "이번 시즌"이라고 하지 않기 위해 받는다. */ season?: string }) {
  return (
    <div className="space-y-7 pb-3">
      <section className="px-4">
        <div className="relative overflow-hidden rounded-[22px] border border-white/70 bg-white/70 px-4 py-4 shadow-[0_18px_48px_-34px_rgba(17,24,39,0.55)] backdrop-blur-md dark:border-white/[0.08] dark:bg-white/[0.035]">
          <div className="absolute inset-x-10 top-0 h-px bg-gradient-to-r from-transparent via-[var(--ud-primary)]/60 to-transparent" />
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <p className="text-[9px] font-black tracking-[0.2em] text-gray-400 dark:text-white/35">
                {season ? `${season} REPORT` : "SEASON REPORT"}
              </p>
              <p className="mt-2 max-w-[240px] text-[15px] font-black leading-snug tracking-tight text-gray-900 dark:text-white">
                {season ? `${season} 시즌` : "이번 시즌"} <span style={{ color: report.primaryRole ? roleColor(report.primaryRole) : undefined }}>{report.primaryRole ?? "여러 역할"}</span>에서 가장 오래 뛰었어요.
              </p>
            </div>
