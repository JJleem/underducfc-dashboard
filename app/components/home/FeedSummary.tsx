...
export const FEED_SUMMARY_ROW =
  "flex min-h-10 w-full items-center gap-2 overflow-hidden py-2 text-left transition-colors active:bg-[var(--ud-primary)]/[0.06] dark:active:bg-[var(--ud-primary)]/[0.04]";

export function FeedSummaryLabel({ children }: { children: ReactNode }) {
  return (
    <span className="w-[68px] shrink-0 text-[12px] font-black text-[#FF718B] dark:text-[var(--ud-primary)]">
      {children}
    </span>
  );
}
  ...
  return (
    <span className="flex shrink-0 items-center gap-2">
      ...
      <ChevronRight
        ...
        strokeWidth={2.4}
        aria-hidden="true"
        className="shrink-0 text-[var(--ud-primary)]/70"
      />
    </span>
  );
}
